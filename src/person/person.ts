import Chromosome, { allosome } from '../chromosome/chromosome';
...
export default class Person {
  ...
  private loadChromosomes(genoType: [string, string]): void {
    ...
    const motherGenes = new ArrayMap<number | symbol, string>();
    const fatherGenes = new ArrayMap<number | symbol, string>();
    ...
    for (const allele of alleles) {
      for (const trait of this.traits) {
        ...
        if (traitChromosomeType === 'X') {
          motherGenes.add(allosome, allele[0]);
          fatherGenes.add(allosome, allele[1]);
        } else if (traitChromosomeType === 'Y')
          fatherGenes.add(allosome, allele[1]);
        else {
    ...
    if (!this.motherChromosomes.find(item => item.isAllosome())) {
      this.motherChromosomes.push(
        new Chromosome({ number: allosome, genes: ['X'] })
      );
    }
